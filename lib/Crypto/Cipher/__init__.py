...
def substitute_bytes(word):
    substituted_word = b''
    for byte in word:
        substituted_word += bytes([S_BOX[byte]])
    return substituted_word

def rotate_word(word, round_const):
...
def expand_key(key):
    ...
    expanded_key = list(words)
     
    for i in range(4, 44):  
        temp = expanded_key[i-1]
        if i % 4 == 0:
            temp = substitute_bytes(rotate_word(temp, rcon(i // 4 - 1))) ^ temp
        expanded_key.append(expanded_key[i-4] ^ temp)
